  ...
  if (action === "crossProjectSummary") {
    ...
    const nodes = await prisma.analysisNode.findMany({
      ...
      include: {
        tree: {
          include: {
            project: { select: { id: true, name: true, client: { select: { name: true, industryMajor: true } } } },
          },
        },
        metrics: { orderBy: { sortOrder: "asc" } },
        children: {
