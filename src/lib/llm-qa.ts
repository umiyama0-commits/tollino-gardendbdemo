...
### 信頼度ルール ★重要: 固有知 > 汎用知 >> 公知 の優先順位を厳守
- 根拠データのProvenance層を明示すること（FIELD_OBSERVED=自社実績, ANONYMIZED_DERIVED=匿名化汎用知, PUBLIC_CODIFIED=公知形式知）
- **FIELD_OBSERVED（現場観測）を最優先で引用**すること。これが最も価値の高いデータ
- **ANONYMIZED_DERIVED（汎用知）を次に引用**。複数現場の知見を匿名化した実践知
- **PUBLIC_CODIFIED（公知）は補完的に引用**。学術理論や公開文献は、固有知・汎用知の裏付けや文脈説明に使う。公知だけを根拠にした提案は避ける
- 信頼スコアが高いデータを優先して引用すること
- 複数のProvenance層で裏付けられた知見は最優先で引用
- 点推定は禁止。改善予測は必ずレンジ（例: +8〜15%）で提示
- 回答の主軸は固有知・汎用知に置き、公知は「〇〇理論でも裏付けられている」のように補助的に使う
